...
fixture_spec="$root_dir/fixtures/kismet-pilot"
kismet_asset="${KISMET_PILOT_ASSET:-$root_dir/../../kismet/dist/gump-handoff/pilot-6/kismet-v0.1.0-gump-pilot.6-x86_64-unknown-linux-gnu}"
...
if [[ ! -x "$kismet_asset" ]]; then
  echo "Missing Linux Kismet pilot asset: $kismet_asset" >&2
  exit 2
fi
actual_sha256="$(shasum -a 256 "$kismet_asset" | awk '{print $1}')"
if [[ "$actual_sha256" != "$expected_sha256" ]]; then
  echo "Kismet pilot checksum mismatch: expected $expected_sha256, got $actual_sha256" >&2
  exit 2
fi
...
output="$root_dir/evidence/kismet-pilot-$capsule_id.capsule"
receipt="$root_dir/evidence/kismet-pilot-$capsule_id.receipt.json"
