...
receipt="${GUMP_FIXTURE_RECEIPT:-$(find "$root_dir/evidence" -name 'ringtail-[0-9]*.receipt.json' -type f -print | sort | tail -1)}"

if [[ -z "$receipt" || ! -f "$receipt" ]]; then
  echo "No Ringtail fixture receipt found; run make fixture-ringtail." >&2
  exit 2
fi
...
result="$(ssh "${ssh_opts[@]}" "manager@$public_ip" \
  "sudo -u gump /usr/local/bin/gump deploy --operation-id '$operation_id' --digest '$digest' --capsule '$remote_capsule' --namespace default --app ringtail --socket /run/gump/gump.sock --wait intent_accepted --format machine")"
deploy_rc=$?
set -e
