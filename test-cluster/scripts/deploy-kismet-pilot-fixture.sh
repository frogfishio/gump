...
receipt="${GUMP_FIXTURE_RECEIPT:-$(find "$root_dir/evidence" -name 'kismet-pilot-[0-9]*.receipt.json' -type f -print | sort | tail -1)}"

if [[ -z "$receipt" || ! -f "$receipt" ]]; then
  echo "No Kismet pilot receipt found; run make fixture-kismet-pilot." >&2
  exit 2
fi
...
if [[ ! -f "$capsule" ]]; then
  echo "Capsule referenced by receipt is missing." >&2
  exit 2
fi
...
for ordinal in 1 2 3; do
  ...
  result="$(ssh "${ssh_opts[@]}" "manager@$public_ip" \
    "sudo -u gump /usr/local/bin/gump deploy --operation-id '$operation_id' --digest '$digest' --capsule '$remote_capsule' --namespace default --app kismet-pilot --socket /run/gump/gump.sock --wait intent_accepted --format machine")"
  deploy_rc=$?
  set -e
