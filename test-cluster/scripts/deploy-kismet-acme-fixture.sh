...
receipt="${GUMP_FIXTURE_RECEIPT:-$(find "$root_dir/evidence" -name 'kismet-acme-pilot-*.receipt.json' -type f -print | sort | tail -1)}"

if [[ -z "$receipt" || ! -f "$receipt" ]]; then
  echo "No Kismet ACME receipt found; build the fixture first." >&2
  exit 2
fi
