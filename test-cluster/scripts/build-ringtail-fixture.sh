...
fixture_spec="$root_dir/fixtures/ringtail"
ringtail_asset="$root_dir/../ext/ringtail/bin/x86_64-unknown-linux-musl/ringtail"
staging="$(mktemp -d)"
...
if [[ ! -x "$ringtail_asset" ]]; then
  echo "Missing Linux Ringtail asset: $ringtail_asset" >&2
  exit 2
fi

install -d "$staging/bin"
install -m 0755 "$ringtail_asset" "$staging/bin/ringtail"
...
output="$root_dir/evidence/ringtail-$capsule_id.capsule"
receipt="$root_dir/evidence/ringtail-$capsule_id.receipt.json"
