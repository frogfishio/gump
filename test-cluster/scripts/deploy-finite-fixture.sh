...
receipt="${GUMP_FIXTURE_RECEIPT:-$(find "$root_dir/evidence" -name 'finite-hello-*.receipt.json' -type f -print | sort | tail -1)}"

if [[ -z "$receipt" || ! -f "$receipt" ]]; then
  echo "No finite fixture receipt found; run make fixture-finite." >&2
  exit 2
fi
...
result="$(ssh "${ssh_opts[@]}" "manager@$public_ip" \
  "sudo -u gump /usr/local/bin/gump deploy --operation-id '$operation_id' --digest '$digest' --capsule '$remote_capsule' --namespace default --app finite-hello --socket /run/gump/gump.sock --wait intent_accepted --format machine")"
deploy_rc=$?
set -e
